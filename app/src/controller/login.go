...
func (h *LoginController) Login(w http.ResponseWriter, r *http.Request) {
	...
	switch request.Type {
	case domain.LoginTypeBasicAuth:
		user, err = h.userRepository.GetByUsername(ctx, nil, request.BasicAuth.Username)
		...
		if err := bcrypt.CompareHashAndPassword([]byte(user.HashPassword), []byte(request.BasicAuth.Password)); err != nil {
		...
		if err := h.cache.SetJSON(ctx, cache.GetMFAReferenceIDKey(referenceID), mfaMetadata, ptr.ToPtr(time.Minute*5)); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	case domain.LoginTypeMFALogin:
		var metadata domain.MFAMetadata
		if err := h.cache.GetJSON(ctx, cache.GetMFAReferenceIDKey(request.MFA.ReferenceID), &metadata); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}

		if metadata.PrivateKey != request.MFA.PrivateKey {
		...
		if err := h.cache.Del(ctx, cache.GetMFAReferenceIDKey(request.MFA.ReferenceID)); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	}
