...
import (
	...
	databaseUtils "adaptive-mfa/utils/database"
	"crypto/sha1"
	"database/sql"
	"encoding/json"
	"fmt"
...
func NewAuthHandler(cache cache.ICache, userRepository repositories.IUserRepository) IAuthHandler {
	return &AuthHandler{
		cache:          cache,
...
func (h *AuthHandler) Register(w http.ResponseWriter, r *http.Request) {
	...
	newUser := models.User{
		...
		Email:        databaseUtils.NewNullString(request.Email),
		Phone:        databaseUtils.NewNullString(request.Phone),
	}
